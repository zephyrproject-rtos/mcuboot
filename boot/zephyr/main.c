...
#if (!defined(CONFIG_XTENSA) && defined(DT_CHOSEN_ZEPHYR_FLASH_LABEL))
    if (!flash_device_get_binding(DT_CHOSEN_ZEPHYR_FLASH_LABEL)) {
        BOOT_LOG_ERR("Flash device %s not found", DT_CHOSEN_ZEPHYR_FLASH_LABEL);
        while (1)
            ;
    }
